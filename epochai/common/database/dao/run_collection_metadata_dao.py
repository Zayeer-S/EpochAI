...
class RunCollectionMetadataDAO:
    ...
    def create_run_metadata(
        ...
        try:
            ...
            params = (
                collection_attempt_id, run_type_id, run_status_id, attempts_successful,
                attempts_failed, config_json, completed_at, current_timestamp
            )
            
            result = self.db.execute_insert_query(query, params)
            
            if result:
                self.logger.info(f"Created run metadata for attempt {collection_attempt_id}: {attempts_successful} successful, {attempts_failed} failed")
                return result
            else:
            ...
            self.logger.error(f"Error creating run metadata for attempt {collection_attempt_id}: {general_error}")
            return None
    ...
    def update_run_completion(
        ...
        try:
            ...
            if affected_rows > 0:
                self.logger.info(f"Updated run {run_id} completion: {attempts_successful} successful, {attempts_failed} failed")
                return True
            else:
    ...
    def get_run_performance_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if results:
                ...
                weighted_duration = sum(row['avg_duration_minutes'] * row['run_count'] for row in results if row['avg_duration_minutes'])
                total_runs_with_duration = sum(row['run_count'] for row in results if row['avg_duration_minutes'])
                
                if total_runs_with_duration > 0:
                    stats['summary']['avg_duration_minutes'] = round(weighted_duration / total_runs_with_duration, 2)
