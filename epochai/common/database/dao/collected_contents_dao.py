...
class CollectedContentsDAO:
    ...
    def create_content(
        ...
        try:
            ...
            if result:
                self.logger.info(
                    f"Created collected content: '{title}' (attempt_id: {collection_attempt_id})",
                )
                return result
    ...
    def get_by_validation_status(
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(contents)} contents with validation status '{validation_status_name}'",
            )
            return contents

        except Exception as general_error:
            self.logger.error(
                f"Error getting contents by validation status '{validation_status_name}': {general_error}",
            )
            return []
