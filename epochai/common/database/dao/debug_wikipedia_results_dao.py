...
class DebugWikipediaResultsDAO:
    ...
    def create_debug_result(
        ...
        try:
            ...
            params = (
                collection_config_id, search_term_used, language_code_used, test_status,
                search_results_json, error_message, test_duration, current_timestamp
            )
            
            result = self.db.execute_insert_query(query, params)
            
            if result:
                self.logger.info(f"Created debug result for '{search_term_used}' ({language_code_used}): {test_status}")
                return result
            else:
    ...
    def get_by_config_id(
        ...
        try:
            ...
            self.logger.error(f"Error getting debug results for config {collection_config_id}: {general_error}")
            return []
