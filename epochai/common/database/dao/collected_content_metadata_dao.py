...
class CollectedContentMetadataDAO:
    ...
    def create_metadata(
        ...
        try:
            ...
            if result:
                ...
                self.logger.error(f"Failed to create metadata '{metadata_key}' for content {collected_content_id}")
                return None
                
        except Exception as general_error:
            self.logger.error(f"Error creating metadata '{metadata_key}' for content {collected_content_id}: {general_error}")
            return None
    ...
    def bulk_create_metadata(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Bulk created {len(metadata_dict)} metadata entries for content {collected_content_id}")
            ...
            self.logger.error(f"Error bulk creating metadata for content {collected_content_id}: {general_error}")
            return 0
    ...
    def search_by_value(
        ...
        query = """
            SELECT * FROM collected_content_metadata WHERE metadata_value ILIKE %s ORDER BY collected_content_id
        """
    ...
    def delete_by_content_id(
        ...
        try:
            ...
            if affected_rows > 0:
                self.logger.info(f"Deleted {affected_rows} metadata entries for content {collected_content_id}")
