...
class LinkAttemptsToRunsDAO:
    ...
    def create_link(
        ...
        try:
            ...
            if result:
                self.logger.info(f"Linked attempt {collection_attempt_id} to run {run_collection_metadata_id}")
                return result
            else:
                self.logger.error(f"Failed to link attempt {collection_attempt_id} to run {run_collection_metadata_id}")
                return None
                
        except Exception as general_error:
            self.logger.error(f"Error linking attempt {collection_attempt_id} to run {run_collection_metadata_id}: {general_error}")
            return None
    ...
    def bulk_link_attempts_to_run(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Bulk linked {len(collection_attempt_ids)} attempts to run {run_collection_metadata_id}")
            ...
            self.logger.error(f"Error bulk linking attempts to run {run_collection_metadata_id}: {general_error}")
            return 0
