...
class DataUtils:
    ...
    def _get_save_variables(
        ...
        incremental_saving: Optional[bool] = None,
        ...
        if validate_before_save and not self.validate_data_structure_and_quality(collected_data):
        ...
        if any(x is None for x in (file_format, filepath, incremental_saving)):
            self.logger.error(
                f"Error one of these three is None: file_format: {file_format} - filepath: {filepath} - incremental_saving: {incremental_saving}",  # noqa
            )
            return None
    ...
    def save_at_end(
        ...
        if incremental_saving:
            self.logger.error(
                f"Save at end being called even though incremental_saving is True: {incremental_saving}",
            )
            return None
        ...
        try:
            ...
            if file_format_lowered == "json":
                ...
                if file_format_lowered != "csv":
                    self.logger.warning(
                        f"File format in config is wrong, currently: {file_format_lowered} - Still saving data to {filepath} as .csv",  # noqa
                    )
    ...
    def log_data_summary(
        ...
        if "language_distribution" in summary:
            self.logger.info("Records by language: ")
                ...
                if isinstance(content, str):
                    ...
                    if len(content.strip()) < min_content_length:
                        validation_errors.append(
                            f"Record {i}: Content too short <{min_content_length} characters.",
                        )
