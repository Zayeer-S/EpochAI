...
class WikipediaPoliticalCollector:
    ...
    def _handle_all_wikipedia_collection(
        ...
        self.logger.info(
            f"Collecting {collection_type} for {len(items_by_language)} items across {len(self.languages)} languages",  # noqa
        )
        ...
        def collect_single_page_data(
            ...
            extra_data = extra_data_func(item) if extra_data_func else None
    ...
    def _save_batch_between_topic_switch(self) -> None:
        ...
        self.logger.info("Saving last items of this batch due to topic change or reaching the end")
        self._save_current_batch()
    ...
    def wikipedia_political_data_orchestrator(self) -> List[Dict[str, Any]]:
        """Collect political data from wikipedia. Orchestrates the collection flow and returns the data"""
